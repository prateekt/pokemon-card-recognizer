...
class TestFramework(unittest.TestCase):
    ...
    def test_pipeline_framework(self) -> None:
        ...
        pipeline = Pipeline.init_from_funcs(
            funcs=[self.append_a, self.append_b, self.reverse, self.reverse], op_class=TextOp
        )
    ...
    def test_parameter_fixing(self) -> None:
        ...
        pipeline = Pipeline.init_from_funcs(
            funcs=[self.append_a, self.append_something, self.reverse, self.reverse], op_class=TextOp
        )
        pipeline.set_pipeline_params(func_name='append_something', params={'something': 'b'})
        self.assertEqual(pipeline.input, None)
        self.assertEqual(pipeline.output, None)
