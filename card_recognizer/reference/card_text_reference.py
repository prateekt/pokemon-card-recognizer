...
def _extract_set_words(cards: List[Card]) -> Dict[int, List[str]]:
    ...
    """
    card_words = {
        card.number: _extract_words_from_card(card) for card in cards
    }
    return card_words
...
def _create_reference_matrix(card_words: Dict[int, List[str]], vocab: Dict[str, int]) -> np.array:
    """
...
def build_set_reference(cards: List[Card], out_pkl_path: Optional[str] = None) -> Tuple[np.array, Dict[str, int]]:
    """
